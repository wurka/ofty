...
def build_from_source(source):
	...
	for line_index, line in enumerate(lines):
		...
		if line.endswith('.png'):
			...
			history[spaces-1].picture = line.strip()
		...
		# это параметр
			...
			try:
				new_parameter = GroupParameter(owner=history[spaces-1], name=name, dimension=dimension)
			except IndexError:
		...
		# это название подгруппы
			...
			if len(history) > 0:
				try:
					new_group = Group(name=line.strip(), parent=history[spaces-1])
				except IndexError:
					raise ValueError(f'[{line_index+1}]: wrong group hierarchy')
			else:
				new_group = Group(name=line.strip())
			new_groups.append(new_group)
			history = history[:spaces] + [new_group]
			if len(history) < spaces + 1:
				raise ValueError(f"[{line_index+1}]: invalid intention")
